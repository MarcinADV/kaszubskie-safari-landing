...
// Landing page for "Kaszubskie Safari" — Buggy & Quady Kaszuby
// Notes for integration:
// - Replace placeholder images with your own.
// - Connect the lead form to your email provider (Brevo/Mailerlite/Mailchimp) or a Google Form webhook.
// - Update contact details and map link.
// - TailwindCSS is assumed to be available. Fonts/colors can be tuned via classes.
// - All texts are in PL and aligned to your offer & pricing (buggy 500 zł / quad 220 zł, 1h trasa z foto-stopem).

// Simple in-page validation + localStorage capture (no backend).
function useLeads() {
...
const features = [
  {
    title: "Trasa Standard 1h",
    price: "Buggy 500 zł / Quad 220 zł",
    desc: "Widokowa pętla A→A z 10-minutowym foto-stopem. Jazda w kolumnie z przewodnikiem.",
    icon: "🚙",
  },
  {
    title: "Extreme 2h",
    price: "Buggy 900 zł / Quad 400 zł",
    desc: "Więcej terenu, piachu i błota. Dłuższa trasa, mocniejsze wrażenia.",
    icon: "⛰️",
  },
  {
    title: "Night Ride 1.5h",
    price: "Buggy 700 zł / Quad 300 zł",
    desc: "Nocne safari z oświetleniem LED i ogniskiem u partnera (opcjonalnie).",
    icon: "🌙",
  },
  {
    title: "Pakiet Rodzinny",
    price: "Zestaw: 1 buggy + 2 quady ≈ 940 zł",
    desc: "Rodzinna wyprawa po kaszubskich szutrach. Zdjęcia/video w cenie pakietu.",
    icon: "👨‍👩‍👧‍👦",
  },
];
...
const routes = [
  {
    name: "Kaszubskie Szutry",
    level: "łatwa",
    desc: "Lekka trasa widokowa. Jeziora, lasy i klasyczne szutry.",
  },
  {
    name: "Kaszubska Dzicz",
    level: "średnia+",
    desc: "Bardziej terenowo: piach, błoto, polany. Dla żądnych adrenaliny.",
  },
  {
    name: "Szlak Jezior Raduńskich",
    level: "łatwa/średnia",
    desc: "Panoramy jezior, punkty widokowe. Instagram-ready.",
  },
  {
    name: "Kaszubskie Safari Nocą",
    level: "średnia",
    desc: "Wieczorna pętla z ogniskiem. Światła LED i klimat 10/10.",
  },
];

export default function KaszubskieSafariLanding() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!validEmail(email)) {
      setStatus({ ok: false, text: "Podaj poprawny adres e‑mail." });
      return;
    }
    if (!consent) {
      setStatus({ ok: false, text: "Zaznacz zgodę na kontakt (RODO)." });
      return;
    }
  ...
  return (
    <main className="min-h-screen bg-white text-slate-900">
          ...
          <div className="grid items-center gap-10 md:grid-cols-2">
            <div>
              ...
              <p className="mt-4 max-w-xl text-slate-200">
                ...
                <span className="font-semibold"> Buggy 500 zł</span> / 
                <span className="font-semibold"> Quad 220 zł</span>.
              </p>
      ...
      <section className="bg-slate-50">
        <div className="mx-auto max-w-7xl px-6 py-10 grid grid-cols-2 gap-6 md:grid-cols-4">
          {[
            ...
            "Legalne trasy szutrowe",
            "Foto‑stop w cenie",
            "Bezpieczeństwo i instruktarz",
          ].map((t, i) => (
            ...
            <form onSubmit={handleSubmit} className="rounded-3xl bg-white/5 p-6 shadow-inner ring-1 ring-white/10">
              <div className="grid gap-4">
                ...
                <div>
                  ...
                  <textarea value={msg} onChange={(e)=>setMsg(e.target.value)} placeholder="Napisz, co Cię interesuje (rodzina, integracja, nocna trasa)…" className="mt-1 h-28 w-full rounded-xl border border-white/20 bg-white/10 px-4 py-3 text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-400" />
                </div>
                ...
                {status && (
                  <p className={`${status.ok ? "text-emerald-300" : "text-rose-300"} text-sm`}>{status.text}</p>
                )}
      ...
      {/* FAQ */}
      <section className="mx-auto max-w-7xl px-6 py-16">
        <h2 className="text-3xl font-bold">FAQ – najczęściej zadawane pytania</h2>
        <div className="mt-8 grid gap-6 md:grid-cols-2">
          {[{
            q: "Czy potrzebuję prawa jazdy?",
            a: "Tak – kierowcy buggy/quadów powinni posiadać ważne prawo jazdy (np. kat. B). Wersje pasażerskie w buggy nie wymagają PJ.",
          },{
            q: "Czy to bezpieczne?",
            a: "Jazda odbywa się w zorganizowanej kolumnie z przewodnikiem i zamykającym. Przed startem masz instruktaż, a w cenie zapewniamy kask i podstawowe wyposażenie ochronne.",
          },{
            q: "Ile trwa wycieczka?",
            a: "Standard to ok. 60 minut (pętla A→A) z 10-minutowym foto‑stopem w połowie. Dostępne są również warianty 1.5–2h.",
          },{
            q: "Gdzie jest baza?",
            a: "Kiełpino / Pępowo Kartuskie – łatwy dojazd z Kartuz i Trójmiasta. Dokładny punkt otrzymasz po rezerwacji.",
          }].map((f) => (
            <div key={f.q} className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-100">
